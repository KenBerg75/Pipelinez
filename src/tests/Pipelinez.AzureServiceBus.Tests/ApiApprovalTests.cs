...
namespace Pipelinez.AzureServiceBus.Tests;
...
public class ApiApprovalTests
{
    ...
    [Fact]
    public void Pipelinez_AzureServiceBus_Public_Api_Matches_Approved_Baseline()
    {
        var approvedPath = GetApprovedPath();
        var actual = ApiApprovalTextGenerator.Generate(typeof(AzureServiceBusPipelineBuilderExtensions).Assembly);
    ...
    private static string GetApprovedPath()
    {
        ...
        return Path.Combine(projectDirectory, "ApprovedApi", "Pipelinez.AzureServiceBus.publicapi.txt");
    }
}
